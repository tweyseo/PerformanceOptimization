...
--[[
    ...
    @Plugin:dumpCache() --> { path:string, method:string, count:number }:table

    @Plugin:rootErrHandlers() --> { errHandler1:function, errHandler2:function, ... }:table

    @Plugin:capture(path, method) --> {
        "middlewares" = { middleware1:function, middleware2:function, ... }:table
        , "errHandlers" = { errHandler1:function, errHandler2:function, ... }:table
        , "handler" = methodHandler:function }:table
]]
