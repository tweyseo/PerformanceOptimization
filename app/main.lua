...
local function f1(_, resp, next) print("handle-test1") resp:send() next(nil) end
local function f2(_, resp, next) print("handle-test2") resp:send() next(nil) end
local function f3(_, resp, next) print("handle-test3") resp:send() next(nil) end

function main.addHandler(app)
    app:get("/test1", f1)
    app:get("/test1/test2", f2)
    app:get("/test1/test2/test3", f3)
end
